...
class Replicator extends Kdyby\Replicator\Container {
    ...
    protected $maxUploads = array(
        'value' => NULL,
        'message' => 'Max. count are %arg, you uploaded %count images.'
    );
    
    /** @var array */
    protected $minUploads = array(
        'value' => NULL,
        'message' => 'Min. count are %arg, you uploaded %count images.'
    );
    
    /** @var array */
    protected $totalSize = array(
        'value' => NULL,
        'message' => 'The size of uploaded files can be up to %arg, given %count.'
    );
    ...
    protected function loadHttpData() {
        ...
        if ($this->minUploads['value']) {
            if ($this->validation['counter'] < $this->minUploads['value']) {
                $this->form->addError(str_replace(array('%arg', '%count'), array($this->minUploads['value'], $this->validation['counter']), $this->minUploads['message']));
            }
        }

        if ($this->maxUploads['value']) {
            if ($this->validation['counter'] > $this->maxUploads['value']) {
                $this->form->addError(str_replace(array('%arg', '%count'), array($this->maxUploads['value'], $this->validation['counter']), $this->maxUploads['message']));
            }
        }
        
        if ($this->totalSize['value']) {
            if ($this->totalSize['value'] < $this->validation['sizes']) {
                $this->form->addError(str_replace(array('%arg', '%count'), array($this->formatBytes($this->totalSize['value'], 0), $this->formatBytes($this->validation['sizes'], 0)), $this->totalSize['message']));
            }
        }
    }
