...
class UploadControl extends Nette\Forms\Controls\UploadControl {
	...
	public function getHttpData($type, $htmlTail = NULL) {
		...
		$checkbox = $this->getForm()
						 ->getHttpData(Nette\Application\UI\Form::DATA_LINE, $this->getCheckboxName(TRUE));
		...
		// Uploading
		$upload = $this->getForm()
					   ->getHttpData($type, $this->getHtmlName() . $htmlTail);
	...
	public function useCheckbox() {
		return $this->default && $this->getStorage()
									  ->get($this->default)
									  ->isExists() === TRUE;
	}
